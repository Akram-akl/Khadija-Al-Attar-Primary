...
## 🤖 أوامر المساعد الذكي (Slash Commands)

هذه الأوامر تُكتب مباشرةً في صندوق الشات مع المساعد الذكي:

| الأمر | الوظيفة |
|-------|---------|
| `/grill-me` | مقابلة تفاعلية لتوضيح متطلبات المشروع قبل البدء — المساعد يطرح أسئلة ذكية لاتخاذ قرارات التصميم معاً |
| `/goal` | تشغيل مهمة طويلة بدون توقف — مفيد للمهام المعقدة التي تحتاج وقتاً طويلاً |
| `/schedule` | جدولة مهمة أو تكرارها على فترات زمنية محددة |
| `/learn` | حفظ تفضيلاتك وتعديلاتك ليتذكرها المساعد في المستقبل |

---

## 🧠 مهارات المساعد الذكي (Skills)

هذه المهارات **تعمل تلقائياً** — لا تحتاج لكتابة أمر خاص، بل يتعرف عليها المساعد من سياق طلبك.

> [!NOTE]
> جميع الـ Skills المذكورة هنا مثبتة **عالمياً** وتعمل مع كل مشاريعك الحالية والمستقبلية.

---

### 🛠️ أدوات التطوير

| الـ Skill | متى يُستخدم؟ |
|-----------|--------------|
| `android-cli` | إنشاء مشاريع Android، نشر التطبيقات، إدارة الـ SDK، تشخيص بيئة التطوير — يعمل عبر سطر الأوامر |
| `uv` | إدارة حزم Python بشكل سريع — يُستخدم كمتطلب أساسي لبعض الـ Skills الأخرى (يثبته تلقائياً إن لم يكن موجوداً) |
| `workflow-skill-creator` | تحويل سير عمل أو خطوات متكررة إلى Skill جاهزة لإعادة الاستخدام — قل للمساعد "اجعل هذا skill" |

---

### 🔬 علوم وبيولوجيا (Science Skills)

> [!NOTE]
> هذه المهارات متخصصة في البحث العلمي والطبي. مفيدة إذا كنت تعمل على مشاريع علمية أو تحتاج بيانات بحثية.

| الـ Skill | متى يُستخدم؟ |
|-----------|--------------|
| `chembl-database` | البحث عن جزيئات دوائية، أهداف علاجية، بيانات IC50/Ki، آليات عمل الأدوية |
| `pubmed-database` | البحث في الأبحاث الطبية المنشورة وجلب الملخصات |
| `literature-search-arxiv` | البحث في الأوراق العلمية والدراسات المسبقة النشر |
| `literature-search-openalex` | قاعدة بيانات أكاديمية شاملة — إحصائيات الاستشهادات والنشر |
| `uniprot-database` | بيانات البروتينات والتعليقات الوظيفية وتعيين المعرّفات |
| `pdb-database` | البنى ثلاثية الأبعاد التجريبية للبروتينات والجزيئات |
| `alphafold-database-fetch-and-analyze` | تحليل البنى المتوقعة للبروتينات بمعرّف UniProt |
| `gnomad-database` | تحليل تكرار المتغيرات الجينية ومقاييس قيود الجينات |
| `clinvar-database` | التصنيف الإكلينيكي للمتغيرات الجينية (Pathogenic/Benign) |
| `clinical-trials-database` | البحث في التجارب السريرية عبر ClinicalTrials.gov |
| `ensembl-database` | ترجمة معرّفات الجينات، تسلسلات الجينوم، تأثيرات المتغيرات |
| `string-database` | تفاعلات البروتين-البروتين ودرجات الثقة |
| `reactome-database` | تحليل المسارات الجزيئية وإثراء قوائم الجينات |
| `pymol` | تصور وتحليل البنى الجزيئية ثلاثية الأبعاد |
...
| الـ Skill | متى يُستخدم؟ |
|-----------|--------------|
| `literature-search-biorxiv` | الأوراق المسبقة النشر في علوم الحياة والطب |
| `literature-search-europepmc` | بحث في Europe PMC وتحميل النصوص الكاملة |
| `ncbi-sequence-fetch` | جلب تسلسلات البروتين والحمض النووي من NCBI |

---

### ⚙️ كيف تطلب استخدام Skill معينة؟

فقط اكتب طلبك بشكل طبيعي:
- `"ابحث لي عن الدواء X في قاعدة ChEMBL"` ← سيستخدم `chembl-database`
- `"أنشئ مشروع Android جديد"` ← سيستخدم `android-cli`
- `"اجعل هذا الـ workflow skill قابلة للإعادة"` ← سيستخدم `workflow-skill-creator`
